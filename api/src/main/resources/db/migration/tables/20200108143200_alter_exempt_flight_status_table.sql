ALTER TABLE exempt_flight_status
    ...
    ALTER COLUMN approach_fees_exempt DROP DEFAULT,
    ...
    ALTER COLUMN aerodrome_fees_exempt DROP DEFAULT,
    ...
    ALTER COLUMN late_arrival_fees_exempt DROP DEFAULT,
    ALTER COLUMN late_arrival_fees_exempt TYPE double precision USING (CASE WHEN late_arrival_fees_exempt IS TRUE THEN 100 ELSE 0 END),
    ALTER COLUMN late_arrival_fees_exempt SET DEFAULT 0,

    ALTER COLUMN late_departure_fees_exempt DROP DEFAULT,
    ALTER COLUMN late_departure_fees_exempt TYPE double precision USING (CASE WHEN late_departure_fees_exempt IS TRUE THEN 100 ELSE 0 END),
...
UPDATE exempt_flight_status SET flight_notes = '' WHERE flight_notes IS NULL;
ALTER TABLE exempt_flight_status ALTER COLUMN flight_notes SET NOT NULL;

--Delete duplicate data
DELETE FROM exempt_flight_status USING exempt_flight_status alias
WHERE exempt_flight_status.flight_item_type = alias.flight_item_type
AND exempt_flight_status.flight_item_value = alias.flight_item_value
AND  exempt_flight_status.id < alias.id;

--Add UNIQUE constraint
ALTER TABLE exempt_flight_status ADD CONSTRAINT unique_flight_item_type_value UNIQUE (flight_item_type, flight_item_value);
