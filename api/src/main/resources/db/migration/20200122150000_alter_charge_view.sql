 ...
 SELECT bc.name,
   ...
   FROM billing_centers bc,
    flight_movements fm
     ...
     LEFT JOIN aerodromes ad ON (fm.dep_ad::text = ad.aerodrome_name::text OR fm.dest_ad::text = ad.aerodrome_name::text)
  ...
  OR ((fm.movement_type::text = 'INT_OVERFLIGHT'::text OR fm.movement_type::text = 'REG_OVERFLIGHT'::text) AND (bc.is_hq = true));
