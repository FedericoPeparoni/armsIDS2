	...
	if (_query_type = 1) then
	 ...
	 if (_query_type = 2) then
        ...
        _query := 'select count(*) as total, 
           ...
           ((select count(*) from flight_movements where movement_type in (''' || _movement_type || ''') ) * 100)::numeric / CASE count(*) WHEN 0 THEN 1 ELSE count(*) END as percent
        from flight_movements ' || _where_cond;
     end if;
     ...
     if (_query_type = 4) then
        ...
        if (_movement_type = 'true') IS TRUE then
	        _query:= 'select count(*) as total,
	        (select count(*) from flight_movements where status in (''PENDING'') and aircraft_type is not null ) as val, 
	        ((select count(*) from flight_movements where status in (''PENDING'') and aircraft_type is not null ) * 100)::numeric / CASE count(*) WHEN 0 THEN 1 ELSE count(*) END as percent
	        from flight_movements';
        else 
	        _query:= 'select count(*) as total,0::bigint as val,0::numeric as percent
	        from flight_movements
	        where status in (''PENDING'') and aircraft_type is null';
        end if;
