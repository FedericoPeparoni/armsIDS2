...
begin
	...
	if (_query_type = 1) then
     ...
     if (_query_type = 9) then
        ...
        where bl.amount_owing > 0 AND bl.invoice_state_type = ('''|| _published ||''')';
     end if;
     ...
     if (_query_type = 10) then
        ...
        0::numeric as percent
        from billing_ledgers bl
        where now() > bl.payment_due_date and bl.invoice_state_type != ('''|| _paid ||''')
         and bl.invoice_state_type != ('''|| _void ||''') ';
     end if;
