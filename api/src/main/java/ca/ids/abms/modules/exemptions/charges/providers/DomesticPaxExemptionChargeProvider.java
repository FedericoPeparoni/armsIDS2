...
/**
 * Domestic PAX charge exemption applies the largest exemption only.
 */
@Component
public class DomesticPaxExemptionChargeProvider implements ExemptionChargeProvider {

    private final LargestExemptionChargeMethod method;

    DomesticPaxExemptionChargeProvider(final LargestExemptionChargeMethod method) {
        this.method = method;
    }
    ...
    public void apply(final FlightMovement flightMovement, final Collection<ExemptionType> exemptions) {
        ...
        Collection<ExemptionCharge> exemptionCharges = exemptions.stream().filter(Objects::nonNull)
            .map(e -> new ExemptionCharge(e.domesticPaxChargeExemption(), e.flightNoteChargeExemption()))
        ...
        ExemptionChargeMethodResult result = method.resolve(new ExemptionChargeMethodModel.Builder()
            .chargeValue(flightMovement.getDomesticPassengerCharges())
        ...
        flightMovement.setDomesticPassengerCharges(result.getAppliedCharge());
        flightMovement.setExemptDomesticPassengerCharges(result.getExemptCharge());
