...
/**
 * Late departure charge exemption applies the largest exemption only.
 */
@Component
public class LateDepartureExemptionChargeProvider implements ExemptionChargeProvider {

    private final LargestExemptionChargeMethod method;

    LateDepartureExemptionChargeProvider(final LargestExemptionChargeMethod method) {
        this.method = method;
    }
    ...
    public void apply(final FlightMovement flightMovement, final Collection<ExemptionType> exemptions) {
        ...
        Collection<ExemptionCharge> exemptionCharges = exemptions.stream().filter(Objects::nonNull)
            .map(e -> new ExemptionCharge(e.lateDepartureChargeExemption(), e.flightNoteChargeExemption()))
        ...
        ExemptionChargeMethodResult result = method.resolve(new ExemptionChargeMethodModel.Builder()
            .chargeCurrency(flightMovement.getLateArrivalDepartureChargesCurrency())
            .chargeValue(flightMovement.getLateDepartureCharges())
        ...
        flightMovement.setLateDepartureCharges(result.getAppliedCharge());
        flightMovement.setExemptLateDepartureCharges(result.getExemptCharge());
