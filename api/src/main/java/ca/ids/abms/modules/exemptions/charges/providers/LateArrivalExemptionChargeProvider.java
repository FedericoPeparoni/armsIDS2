...
/**
 * Late arrival charge exemption applies the largest exemption only.
 */
@Component
public class LateArrivalExemptionChargeProvider implements ExemptionChargeProvider {

    private final LargestExemptionChargeMethod method;

    LateArrivalExemptionChargeProvider(final LargestExemptionChargeMethod method) {
        this.method = method;
    }
    ...
    public void apply(final FlightMovement flightMovement, final Collection<ExemptionType> exemptions) {
        ...
        Collection<ExemptionCharge> exemptionCharges = exemptions.stream().filter(Objects::nonNull)
            .map(e -> new ExemptionCharge(e.lateArrivalChargeExemption(), e.flightNoteChargeExemption()))
        ...
        ExemptionChargeMethodResult result = method.resolve(new ExemptionChargeMethodModel.Builder()
            ...
            .chargeValue(flightMovement.getLateArrivalCharges())
        ...
        flightMovement.setLateArrivalCharges(result.getAppliedCharge());
        flightMovement.setExemptLateArrivalCharges(result.getExemptCharge());
