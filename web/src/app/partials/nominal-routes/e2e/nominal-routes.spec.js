'use strict';

describe('nominal-routes view', () => {
    let page;

    let mock = require('protractor-http-mock');

    beforeEach(() => {
        browser.get('/#/nominal-routes');
        page = require('./nominal-routes.po');
        mock(['nominal-routes/e2e/nominal-routes.mock']);
    });

    afterEach(() => {
        mock.teardown();
    });

    describe('form', () => {

        function fillOutForm() {
            page.formFields['type'].sendKeys('FIR / FIR');
            page.formFields['pointa'].sendKeys('A123');
            page.formFields['pointb'].sendKeys('B123');
            page.formFields['nominal_distance'].sendKeys('56.89');
        }

        function popup() {
            let EC = protractor.ExpectedConditions;
            let confirmButton = element(by.buttonText('Confirm'));
            browser.wait(EC.visibilityOf(confirmButton), 5000);
            confirmButton.click();
        }

        // RESET Button and Form Fields
        it('should clear fields when the reset button is clicked', () => {
            fillOutForm();

            page.formFields['type'].getAttribute('value').then((value) => {
                expect(value).toBe('string:FIR / FIR');
            });
            page.formFields['pointa'].getAttribute('value').then((value) => {
                expect(value).toBe('A123');
            });
            page.formFields['pointb'].getAttribute('value').then((value) => {
                expect(value).toBe('B123');
            });
            page.formFields['nominal_distance'].getAttribute('value').then((value) => {
                expect(value).toBe('56.89');
            });

            page.resetButtonEl.click();

            for (let field = 0; field < page.formFields.length; fields++) {
                page.formFields[page.formFields[field]].getAttribute('value').then((value) => {
                    expect(value).toBe('');
                });
            }
        });

        // CREATE Button
        describe('create', () => {
            it('shouldn\'t be possible to click when the form is empty', () => {
                page.createButtonEl.getAttribute('disabled').then((value) => {
                    expect(value).toBeTruthy();
                });
            });

            it('should be enabled when all fields are filled out', () => {
                fillOutForm();
                page.createButtonEl.getAttribute('disabled').then((value) => {
                    expect(value).toBeFalsy();
                });
            });

            it('should create a nominal route when the form is submitted', () => {
                fillOutForm();
                page.createButtonEl.click();
                browser.waitForAngular();
            });
        });

        // UPDATE Button
        describe('update', () => {
            it('shouldn\'t be visible if a nominal route is not selected', () => {

                page.updateButtonEl.isDisplayed().then((value) => {
                    expect(value).toBeFalsy();
                });
            });

            it('should be visible if a nominal route is selected', () => {
                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();

                    page.updateButtonEl.isDisplayed().then((value) => {
                        expect(value).toBeTruthy();
                    });
                });
            });

            it('should be enabled if a nominal route is selected', () => {
                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();
                    fillOutForm();

                    page.updateButtonEl.getAttribute('disabled').then((value) => {
                        expect(value).toBeFalsy();
                    });
                });
            });

            it('should update when the nominal route is updated', () => {
                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();

                    page.formFields['pointa'].clear().sendKeys('ABC');

                    page.formFields['pointa'].getAttribute('value').then((value) => {
                        expect(value).toBe('ABC');
                    });

                    el[0].click();
                    page.formFields['pointa'].clear().sendKeys('PointA');

                    page.updateButtonEl.click();

                    popup();
                });

                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();

                    page.formFields['pointa'].getAttribute('value').then((value) => {
                        expect(value).toBe('PointA');
                    });
                });
            });
        });

        // DELETE Button
        describe('delete', () => {
            it('shouldn\'t be visible if a nominal route is not selected', () => {
                page.deleteButtonEl.isDisplayed().then((value) => {
                    expect(value).toBeFalsy();
                })
            });

            it('should be visible if a nominal route is selected', () => {
                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();

                    page.deleteButtonEl.isDisplayed().then((value) => {
                        expect(value).toBeTruthy();
                    });
                });
            });

            it('should be removed upon clicking delete', () => {
                element.all(by.repeater('item in list')).then((el) => {
                    el[0].click();

                    page.deleteButtonEl.click().then(() => {
                        popup();

                        let type = page.formFields.type.getAttribute('value');
                        type.then((val) => {
                            expect(val).toBe('');
                        });
                    });
                });
            });
        });

        // TABLE Filter
        describe('filter', () => {
            it('should filter all from table if a letter is an input', () => {

                page.textFilter.sendKeys('zzzz');

                element.all(by.repeater('item in list')).then((el) => {
                    expect(el[0]).toBeUndefined();
                });
            });
        });

    }); //End suite

}); //End view